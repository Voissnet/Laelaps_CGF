...
public class SMSCSessionImpl extends SmppObject implements SMSCSession {
     private static final Logger LOGGER = Logger.getLogger(SMSCSessionImpl.class.getName());
	...
	public void stop() {
		debug.write("SMSCSession stopping");
                LOGGER.log(WARNING, "CGF SMSCSession stopping() ");
		keepReceiving = false;
	}
	...
	public void run() {
		...
		debug.write("SMSCSession starting receiver");
                //LOGGER.log(WARNING, "CGF SMSCSession starting receiver");
		receiver.start();
		isReceiving = true;
		try {
			while (keepReceiving) {
				try {
                                    //LOGGER.log(SEVERE, "Receiving PDU " );
                                    debug.write("SMSCSession going to receive a PDU");
                                    //LOGGER.log(WARNING, "SMSCSession going to receive a PDU");
					pdu = receiver.receive(getReceiveTimeout());
				} catch (Exception e) {
                                        //LOGGER.log(SEVERE, "SMSCSession caught exception receiving PDU " , e.getMessage() );                                    
					debug.write("SMSCSession caught exception receiving PDU " + e.getMessage());
                                        //LOGGER.log(WARNING, "CGF SMSCSession caught exception receiving PDU " + e.getMessage());
				}
				...
				if (pdu != null) {
					...
					if (pdu.isRequest()) {                                          
						debug.write("SMSCSession got request " + pdu.debugString());
                                                 //LOGGER.log(WARNING, "CGF SMSCSession got request: " + pdu.debugString());
						pduProcessor.clientRequest((Request) pdu);
					} else if (pdu.isResponse()) {
						debug.write("SMSCSession got response " + pdu.debugString());
                                                //LOGGER.log(WARNING, "CGF SMSCSession got response " + pdu.debugString());
						pduProcessor.clientResponse((Response) pdu);
					} else {
						debug.write("SMSCSession not reqest nor response => not doing anything.");
                                                //LOGGER.log(WARNING, "CGF SMSCSession not request nor response => not doing anything.");
					}
				} else {
					timeoutCntr++;
					if (timeoutCntr > 5) {
						debug.write("SMSCSession stoped due to inactivity");
                                                //LOGGER.log(WARNING, "CGF SMSCSession stoped due to inactivity");
						stop();
					}
				}
			}
			...
			isReceiving = false;
                        //LOGGER.log(WARNING, "CGF setting isReceiving to false");
		}
		debug.write("SMSCSession stopping receiver");
                //LOGGER.log(WARNING, "CGF SMSCSession stopping receiver");
		receiver.stop();
		debug.write("SMSCSession exiting PDUProcessor");
		pduProcessor.exit();
		try {
			debug.write("SMSCSession closing connection");
                        //LOGGER.log(WARNING, "CGF SMSCSession closing connection");
			connection.close();
		} catch (IOException e) {
                        //LOGGER.log(WARNING, "CGF Exception closing SMSCSession's connection: " + e.getMessage());
			event.write(e, "closing SMSCSession's connection.");
		}
                //LOGGER.log(WARNING, "CGF SMSCSession exiting run() ");
	...
	public void send(PDU pdu) throws IOException, PDUException {
		...
		debug.write("SMSCSession going to send pdu over transmitter");
                //LOGGER.log(WARNING, "SMSCSession going to send pdu over transmitter");
		try {
         	    transmitter.send(pdu);
                    LOGGER.log(WARNING, "SMSCSessionImpl PDU transmitted ok ");
		} catch (IOException e) {
                    LOGGER.log(SEVERE, "SMSCSession caught exception sending PDU: "+ e.getMessage() );                                    
		    debug.write("SMSCSession caught exception receiving PDU " + e.getMessage());
