...
import static net.redvoiss.sms.dao.DAO.LOGGER;
...
public interface SMSService {
    ...
    default MessageStatus checkMessageStatus(int messageId, String username, DataSource ds) throws SMSError {
        ...
        try (java.sql.Connection conn = ds.getConnection()) {
            try (java.sql.PreparedStatement ps = conn.prepareStatement(
                    "SELECT LM.ESTADO, LI.ID_CLIENTE FROM FC_SMS_LOTE_MENSAJE LM, FC_SMS_LOTE L, FC_SMS_LOTE_ITEM LI, FC_USUARIO U WHERE L.USUA_COD = U.USUA_COD AND LM.LOTE_COD = L.LOTE_COD AND LM.MENS_COD = ? AND U.USERNAME = ? AND LI.LOTE_COD=L.LOTE_COD ")) {
                ...
                try (java.sql.ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        ret = new MessageStatus(messageId, mapStatus(state), idcliente);
    ...
    default List<BatchStatus> checkBatchStatus(int batchId, String username, DataSource ds) throws SMSError {
        ...
        try (java.sql.Connection conn = ds.getConnection()) {
            ...
            String sql = "select LM.MENS_COD, LM.SMS_COD, LM.DESTINO, LM.ESTADO, LI.ID_CLIENTE " +
                ...
                try (java.sql.ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (status == null) {
                            ...
                            ret.add(new BatchStatus(msgCod, destination, status, idcliente));
                        }
                    }
                }
            }
                                        ...
                                        if (i + 1 == arr.length && !"".equals(fallidos)) {
                                            if (reply) {
                                                ret = net.redvoiss.sms.dao.DAO.creaDespachaLoteCompletoReply(
                                                        ...
                                                        new String(msg.getBytes("UTF-8"), "ISO-8859-1"), "", ret, false, //i + 1 == arr.length,
    ...
    default String send_v3(final String username, BulkMessage bulkMessage, DataSource dataSource, boolean reply)
            throws SMSError {
        String ret = null;
            ...
            if (dataSource == null) {
                ...
                try (Connection conn = dataSource.getConnection()) {
                    ...
                    if (arr == null) {
                        ...
                        LOGGER.warning("Number of destinations: "+arr.length);
                        for (int i = 0; i < arr.length; i++) {
                            Message m = arr[i];
                            if (m == null) {
                                LOGGER.warning("Discarding message due to detail is missing");
                                ...
                                String msg = bulkMessage.getMessage(); //new String (bulkMessage.getMessage().getBytes(),"ISO-8859-1");
                                LOGGER.warning(() -> String.format( "Validating destination: {%s}", m.getDestination()) );
                                if (!"".equals(msg) && destination.isOK_v2()) {
                                    if (msg.length()>MAX_SMS_LEN)
                                       msg = ""+msg.substring(0, MAX_SMS_LEN );
                                    LOGGER.warning(() -> String.format( "SMS con idCliente: {%s}", m.getIdcliente()) );
                                    try {
 
                                        if (i + 1 == arr.length && !"".equals(fallidos)) {

                                            if ((int)msg.length()>160) {
                                                // Es un mensaje largo, debo construir los bytes del UdhHeader                                                
                                                ret = net.redvoiss.sms.dao.DAO.creaDespachaLoteCompletoLargo
                                                       (
                                                        conn, 
                                                        username, destination.getTarget(), 
                                                        msg, "", ret, false, //i + 1 == arr.length,
                                                        bulkMessage.isCommercial(),
                                                        bulkMessage.getSendDate() == null ? new Date() : bulkMessage.getSendDate(),
                                                        m.getIdcliente(),
                                                        (byte)i
                                                       ); 
                                            } else {
                                        ...
                                        /**
                                         * We cannot simply avoid calling these methods when 'field' if unavailable, as this is an alternative parameter.
                                         * Unfortunately, it is the combination of 'message' and 'field', being replaced at DB side, that fails. Ref:
                                         * LAEL-168
                                         */
                                    ...
                                    if (reply) {
                                        LOGGER.warning(() -> String.format(
                                            "Calling creaDespachaLoteConItemFallidoReply to destination: {%s}", m.getDestination()));
                                        ...
                                        LOGGER.warning(() -> String.format(
                                            "Calling creaDespachaLoteConItemFallido to destination: {%s}", m.getDestination()));
                                        ret = net.redvoiss.sms.dao.DAO.creaDespachaLoteConItemFallido(conn,
                                                    username, destination.getTarget(), bulkMessage.getMessage(),
                                                    "", ret, i + 1 == arr.length, bulkMessage.getBulkName(),
                                                    m.getField(), bulkMessage.isCommercial(),
