...
public class SMSCSessionImpl extends SmppObject implements SMSCSession {
	...
	public void run() {
		...
		try {
			while (keepReceiving) {
				try {
					debug.write("QA SMSCSession going to receive a PDU");
                                        LOGGER.log(WARNING, "CGF SMSCSession going to receive a PDU");
					pdu = receiver.receive(getReceiveTimeout());
				} catch (Exception e) {
                                        LOGGER.log(WARNING, "QA CGF SMSCSession caught exception receiving PDU " + e.getMessage());
				...
				if (pdu != null) {
					timeoutCntr = 0;
					if (pdu.isRequest()) {
						debug.write("QA SMSCSession got request " + pdu.debugString());
                                                LOGGER.log(WARNING, "QA CGF SMSCSession got request: " + pdu.debugString());
						pduProcessor.clientRequest((Request) pdu);
					} else if (pdu.isResponse()) {
                                                LOGGER.log(WARNING, "QA CGF SMSCSession receiving response: " + pdu.debugString());
						debug.write("QA SMSCSession got response " + pdu.debugString());
		...
		try {
			...
			connection.close();
                        ...
                        LOGGER.log(WARNING, "CGF Exception on closing SMSCSession's connection:"+ e.getMessage());
