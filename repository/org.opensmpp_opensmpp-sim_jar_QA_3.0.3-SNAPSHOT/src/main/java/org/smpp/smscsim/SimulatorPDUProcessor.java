...
public class SimulatorPDUProcessor extends PDUProcessor {
 ...
 private static final Logger LOGGER = Logger.getLogger(SimulatorPDUProcessor.class.getName());        
	 ...
	 */
	private static final String SYSTEM_ID = "Smsc Redvoiss QA";
	...
	public void clientRequest(Request request) {
		debug.write("QA SimulatorPDUProcessor.clientRequest() " + request.debugString());
		Response response;
		int commandStatus;
		int commandId = request.getCommandId();
		try {
			display("QA client request: " + request.debugString());
			if (!bound) { // the first PDU must be bound request
				if (commandId == Data.BIND_TRANSMITTER
					...
					if (commandStatus == 0) { // authenticated
                                                display("QA a BIND, and  deliveryInfoSender.getIsbound(): " +deliveryInfoSender.getIsbound() );                                          

                                                /*if (deliveryInfoSender!=null && deliveryInfoSender.getIsbound()==true) {
                                                    deliveryInfoSender.setIsbound(false);
                                                    display("QA I will stop deliveryInfoSender, with deliveryInfoSender.getIsbound(): " +deliveryInfoSender.getIsbound());                                          
						...
						// success => bound
                                                /*if (deliveryInfoSender!=null && deliveryInfoSender.getIsbound()==false) {
                                                    ...
                                                    display("QA I will start deliveryInfoSender, NOW with deliveryInfoSender.getIsbound(): " +deliveryInfoSender.getIsbound() );                                          
				...
				if (request.canResponse()) {
					...
					switch (commandId) { // for selected PDUs do extra steps
						case Data.SUBMIT_SM :
							...
							display("putting QA message into message store");
							try {
					...
					if (commandId == Data.UNBIND) {
						// unbind causes stopping of the session
                                                display("Setting QA bound to 'false' for : "+ request.debugString());
                                                //bound = false;
						session.stop();
					}
				} else {
	...
	public void clientResponse(Response response) {
		debug.write("QA SimulatorPDUProcessor.clientResponse() " + response.debugString());
		display("QA client response: " + response.debugString());               
	}
	...
	public void serverRequest(Request request) throws IOException, PDUException {
		//debug.write("QA SimulatorPDUProcessor.serverRequest() " + request.debugString());
		//session.send(request);
                /*if (deliveryInfoSender!=null && deliveryInfoSender.getIsbound()==false) {
                   display("En serverRequest(), deliveryInfoSender.getIsbound()): " + deliveryInfoSender.getIsbound()+", bound="+bound);
                   return;
                } else
                   display("---> En serverRequest(), deliveryInfoSender.getIsbound()): " + deliveryInfoSender.getIsbound()+", bound="+bound);
                */
			  ...
			  || commandId == Data.UNBIND_RESP
                ...
                {
                    display("QA server request (1): " + request.debugString());
		    //display("server request (2): " + request.debugString());
		    
		    try {
         	        session.send(request);
                        display( "QA server request sent ok ");
		    } catch (IOException e) {
                        if (deliveryInfoSender!=null ) {
                            deliveryInfoSender.setIsbound(false);
                            bound = false;
                        ...
                        display( "QA serverRequest caught exception sending request: "+ e.getMessage() );   
	...
	public void serverResponse(Response response) throws IOException, PDUException {
		debug.write("QA SimulatorPDUProcessor.serverResponse() " + response.debugString());
		display("QA server response: " + response.debugString());
	...
	protected int checkIdentity(BindRequest request) {
		...
		if (user != null) {
			...
			if (password != null) {
				...
				display("not authenticated (2) " + systemId + " -- no password for user.");
			}
		} else {
			commandStatus = Data.ESME_RINVSYSID;
			debug.write("system id " + request.getSystemId() + " not authenticated -- not found");
			display("not authenticated (3) " + request.getSystemId() + " -- user not found");
		}
