...
public class DeliveryInfoSender extends ProcessingThread {
	...
	protected void deliver(DeliveryInfoEntry entry) throws UnsupportedEncodingException, IOException, PDUException {
		debug.enter(this, "deliver");
                
                /*if (getIsbound()==false) {
                    LOGGER.warning("--> En DeliveryInfoSender.deliver(), connection SMPP NOT BOUND");
                    return;
                } *///else 
                //    LOGGER.warning("--> En DeliveryInfoSender.deliver(), connection SMPP IS BOUND");
		...
		try {
                    ...
                    {
                        try {
                                //LOGGER.warning("Me dormire en DeliveryInfoSender.deliver():"+DLR_DELAY+" milisegundos");
                                Thread.sleep(DLR_DELAY);
                                //LOGGER.warning("QA I slept in DeliveryInfoSender.deliver():"+DLR_DELAY+" milisegundos");
			...
			entry.processor.serverRequest(deliver);
                        LOGGER.warning( "QA deliver sent serverRequest DELIVER ok ");
                        
		    } catch (IOException e) {
                        //setIsbound(false);
                        LOGGER.warning( "QA DeliveryinfoSender.deliver() caught IOException from entry.processor.serverRequest(deliver) " );   
                        throw new IOException();
		    } catch (PDUException e) {
                        //setIsbound(false);
                        LOGGER.warning( "QA DeliveryinfoSender caught PDUException from entry.processor.serverRequest(deliver) " );   
	...
	public void process() {
                ...
                //LOGGER.warning("En DeliveryInfoSender.process() , DLR_DELAY:"+DLR_DELAY+" milisegundos");
		...
		if (submitRequests.isEmpty()) {
			...
			while (!submitRequests.isEmpty()) {
                        ...
                        /*if (nDLRs>=MAX_DLR)
                        {
                            try {
                                Thread.sleep(10000);
                                LOGGER.warning("I sleep 10 seconds in DeliveryInfoSender");
                            } catch (InterruptedException e) {
                                        //e.printStackTrace();
                            }
                        } */     
