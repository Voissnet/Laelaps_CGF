...
//import static org.smpp.pdu.ByteData.decodeUnsigned;
...
private static final Logger LOGGER = Logger.getLogger(SubmitSM.class.getName());  
	...
	private byte esmClass = Data.DFLT_ESM_CLASS; //  Data.SM_UDH_GSM; // SM_UDH_GSM seria para hacer que concatene SMS largos >160
	...
	public void setBody(ByteBuffer buffer)
		...
		destAddr.setData(buffer); // ?
                ...
                final short esmclass = decodeUnsigned(buffer.removeByte());
                //LOGGER.warning("En setBody(), esmclass="+esmclass);
                if (esmclass>0)
        	    setEsmClass((byte)0x40); //buffer.removeByte());
                else
	            setEsmClass((byte)0x00); //buffer.removeByte());
		...
		if( ( getEsmClass() & Data.SM_UDH_GSM) == Data.SM_UDH_GSM ) { //Concatenated SMS
			...
			final short udhLength = decodeUnsigned(buffer.removeByte());
				...
				final short msgPart = decodeUnsigned(buffer.removeByte());
                        ...
                        //LOGGER.warning("SMS Particionado, smLength parcial="+smLength+", udhLength="+udhLength+", ieIdentifier="+ieIdentifier+", ieDataLength="+ieDataLength +", msgId="+msgId+", msgParts="+msgParts+", msgPart="+msgPart);

                        setUdhHeader((byte)udhLength,(byte)ieIdentifier,(byte)ieDataLength,(byte)msgId,(byte)msgParts,(byte)msgPart); //(byte[])buffer.removeBuffer(udhLength).getBuffer(), udhLength);
			...
			/*if( true ) { 
				...
				final short msgPart = decodeUnsigned(buffer.removeByte());
                                ...
                                LOGGER.warning("A chunk: ieIdentifier="+ieIdentifier+", ieDataLength="+ieDataLength +", msgId="+msgId+", msgParts="+msgParts+", msgPart="+msgPart);
			}*/
		} else {
			setSmLength(decodeUnsigned(buffer.removeByte()));
		}
                //LOGGER.warning("En setBody(), seteo el SMS, getSmLength()="+getSmLength());
                
                if ( esmclass>0 )
                {
		    /*ByteBuffer ed = new ByteBuffer();
		    ed.appendByte((byte) udhHeader[0]);  
		    ed.appendByte((byte) udhHeader[1]);  
                    ed.appendByte((byte) udhHeader[2]);  
		    ed.appendByte((byte) udhHeader[3]); 
		    ed.appendByte((byte) udhHeader[4]); 
		    ed.appendByte((byte) udhHeader[5]); 
                    String s = buffer.removeBuffer(getSmLength()).toString();
                    //LOGGER.warning("---> s="+s); 
		    ed.appendString(s);          
                    LOGGER.warning("---> ed='"+ed+"'"); */
                    shortMessage.setData(buffer.removeBuffer(getSmLength()), getSourceAddr().getAddress(), getDataCoding());

                } else {                                          
     		    shortMessage.setData(buffer.removeBuffer(getSmLength()),getSourceAddr().getAddress(), getDataCoding());
                
                }
                //LOGGER.warning("====> Y el SMS queda en debugString()="+debugString()); 
	}
	...
	public void setShortMessageData(ByteBuffer value)
		throws PDUException, NotEnoughDataInByteBufferException, TerminatingZeroNotFoundException {
                //LOGGER.severe("En setShortMessageData_0() "); 
		shortMessage.setData(value,getSourceAddr().getAddress(),getDataCoding());
		setSmLength((short) shortMessage.getLength());
	}

	public void setShortMessage(String value) throws WrongLengthOfStringException {
            //LOGGER.severe("En setShortMessage_0() SIN encoding,  con Address="+getSourceAddr().getAddress()); 
	...
	public void setShortMessage(String value, String encoding)
		throws WrongLengthOfStringException, UnsupportedEncodingException {
                //LOGGER.severe("En setShortMessage_1() con encoding ="+encoding+",  con Address="+getSourceAddr().getAddress()); 
	...
	public void setSourceAddr(String address) throws WrongLengthOfStringException,UnsupportedEncodingException 
        {
	...
	public void setDataCoding(byte value) {          
                //LOGGER.severe("-------------> En setDataCoding con value='"+value+"' ");             
		dataCoding = value;
	}
	...
	public void setUdhHeader(byte udhLen, byte ieIdentifier, byte ieDataLength, byte msgId, byte msgParts, byte msgPart)   {
            udhLength = (short)udhLen;
            udhHeader = new byte[udhLen+1];
            udhHeader[0] = (byte) udhLen;
            udhHeader[1] = (byte) ieIdentifier;
            udhHeader[2] = (byte) ieDataLength;
            udhHeader[3] = (byte) msgId;
            udhHeader[4] = (byte) msgParts;
            udhHeader[5] = (byte) msgPart;                               
	}
        
        public byte[] getUdhHeader()   {
                ByteBuffer ed = new ByteBuffer();
		    ed.appendByte((byte) udhHeader[0]);  
		    ed.appendByte((byte) udhHeader[1]);  
                    ed.appendByte((byte) udhHeader[2]);  
	...
	public String getShortMessage() throws UnsupportedEncodingException {
            //LOGGER.severe("En getShortMessage_0()  con Address="+getSourceAddr().getAddress()); 
		return shortMessage.getMessage(getSourceAddr().getAddress());
	}
	public String getShortMessage(String encoding) throws UnsupportedEncodingException {
               //LOGGER.severe("En getShortMessage_1() llamo a getMessage(" + encoding +", "+getSourceAddr().getAddress()+")"); 
               return shortMessage.getMessage(encoding,getSourceAddr().getAddress());  
	}
