			...
			if (connType == CONN_CLIENT) {
				try {
					...
					if( System.getProperty("bind.address") != null ) {                                             
                                            // CGF, 20231204, I replaced this for specifying the NetworkInterface to use 
                                            // when the server changed IP address, keeping old network interface still alive and running
					    // socket.bind( new java.net.InetSocketAddress( java.net.InetAddress.getByName(System.getProperty("bind.address")), 0) );
                                            // ens161 is the new network interface, with new IP address 
                                            NetworkInterface nif = NetworkInterface.getByName("ens161");  
					    Enumeration<InetAddress> nifAddresses = nif.getInetAddresses();

					    socket = socketFactory.createSocket();  //new Socket();
					    socket.bind(new java.net.InetSocketAddress(nifAddresses.nextElement(), 0));
