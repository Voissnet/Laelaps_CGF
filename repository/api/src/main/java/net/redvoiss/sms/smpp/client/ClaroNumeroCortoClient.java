...
public class ClaroNumeroCortoClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(ClaroNumeroCortoClient.class.getName());
	...
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "2"));// defaults to Claro's route
	private static final String IDOIDD = System.getProperty("idoidd", "310219");
	private static final int SLEEP_SECS = 1;
	private static final String QUERY = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_con_nc");  // leo ta,bien esm_class y udh_jeader
	 ...
	 * @see FileDebug
	 */
	static Debug debug = new FileDebug(dbgDir, "claro.dbg");
	 ...
	 * @see FileEvent
	 */
	static Event event = new FileEvent(dbgDir, "claro.evt");
	 ...
	 */
	public ClaroNumeroCortoClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final ClaroNumeroCortoClient test = new ClaroNumeroCortoClient()) {
			test.bind();
			if (test.bound) {
							...
							while (rs.next()) {
								...
								try {
									...
									if (contentMatcher.matches()) {
										final int route = rs.getInt("CODE_RUTA");
										if (route == 2) {// Classic mode enabled
											destAddress = new Address((byte) 0, (byte) 13,
													destination.replaceFirst(PREFIX_REGEX, "310219"));
										} else if (route == 24) {
											destAddress = new Address((byte) 0, (byte) 13,
													destination.replaceFirst(PREFIX_REGEX, "310320"));
										} else {
										...
										for (int i = 0; i < count; i++) {
											...
											try {
												if (asynchronous) {
													...
													m_smsCodeFromSequenceMap.put(seq,
															new OriginCode(smsCode, msgIdGW, route));
						...
						case Data.DELIVER_SM:
							...
							if (dlrInfo == null) {
								...
								if (deliver.hasReceiptedMessageId()) {
									...
									if (dlrInfo.getId() == null) {
										LOGGER.warning(String.format("Message does not comply DLR format: {%s}", dlr));
									} else {
										receiptedMessageId = Long.toHexString(Long.parseLong(dlrInfo.getId()))
												.toUpperCase();
									}
								}
								if (receiptedMessageId == null) {
									...
									try (java.sql.Connection conn = m_ds.getConnection()) {
										try {
											...
											if (x == null) {
												try (java.sql.PreparedStatement ps = conn.prepareStatement(
														"select e.sms_cod, e.msgid_gw, e.code_ruta from fs_esme e where e.code_smsc = ? and (e.code_ruta = ? or e.code_ruta = ?)")) {
													...
													try (java.sql.ResultSet rs = ps.executeQuery()) {
														if (rs.next()) {
															x = new OriginCode(rs.getString("sms_cod"),
																	rs.getString("msgid_gw"), rs.getInt("code_ruta"));
	...
	private static void store(java.sql.Connection conn, OriginCode x, String receiptedMessageId, String doneDate)
			throws SQLException {
		try (java.sql.PreparedStatement ps = conn.prepareStatement(
				"insert into FS_ESME_DATA (select e.sms_cod, e.dst_addrs, e.sms_txt, e.fecha_hora, e.code_ruta, e.code_smsc, 0 /*NO ERROR (legacy)*/, TO_DATE(?, 'YYMMDDHH24MI'), e.sourc_addrs from fs_esme e where e.code_smsc = ? and (e.code_ruta = ? or e.code_ruta = ?) )")) {
