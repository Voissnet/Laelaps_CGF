...
public class HablaIpClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(HablaIpClient.class.getName());
	...
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "38"));// defaults to HablaIP's route
	private static final String IDOIDD = ""; //System.getProperty("idoidd", "310389");
	 ...
	 * @see FileDebug
	 */
	static Debug debug = new FileDebug(dbgDir, "hablaip.dbg");
	 ...
	 * @see FileEvent
	 */
	static Event event = new FileEvent(dbgDir, "hablaip.evt");

	/**
	 * File with default settings for the application.
	 */
	static String propsFilePath = System.getProperty("smpp.default.cfg", "./hablaip.cfg");
	 ...
	 */
	public HablaIpClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final HablaIpClient test = new HablaIpClient()) {
			test.bind();
			if (test.bound) {
		...
		try {
			...
			if (syncMode.compareToIgnoreCase("a") == 0) {
				...
				return;
                        ...
                        LOGGER.warning("Sync Mode: " + syncMode+", asynchronous="+asynchronous);
			// input values
			...
			port = getParam("Port number", port);
                        ...
                        LOGGER.warning("Binding to IP address " + ipAddress+" by port "+port);
			...
			password = getParam("Your password", password);
                        ...
                        LOGGER.warning("Binding with systemID " + systemId+" and password "+password);
								...
								try {
									...
									if (contentMatcher.matches()) {
										if (IDOIDD.length() == 0) {// Classic mode enabled
											...
											destAddress = new Address((byte) 0, (byte) 15, 
										...
										for (int i = 0; i < count; i++) {
											...
											try {
												if (asynchronous) {
													...
													LOGGER.log(WARNING, "Submit response {0}", response.debugString());
													messageId = response.getMessageId();
                                                                                                        //LOGGER.log(WARNING, "Submit response MessageId {0} ", messageId);
												}
                                                                                                String sql = "UPDATE FS_ESME SET FECHA_HORA=sysdate, STATUS="+DAO.MessageState.PENDING.getState()+", CODE_SMSC='"+messageId+"' where SMS_COD="+smsCode;
                                                                                                LOGGER.log(WARNING, "Submit response MessageId "+messageId+", SQL "+sql);
												
												//try (java.sql.PreparedStatement _ps = conn.prepareStatement("UPDATE FS_ESME SET FECHA_HORA=sysdate, STATUS=?, CODE_SMSC=? where SMS_COD=?")) {
												try (java.sql.PreparedStatement _ps = conn.prepareStatement(sql)) {
													//_ps.setString(1, DAO.MessageState.PENDING.getState());
													//_ps.setString(2, messageId);
													//_ps.setString(3, smsCode);
													int rowCount = _ps.executeUpdate();
													assert rowCount > 0;
												}
	...
	private void enquireLink() {
		...
		try {
			...
			EnquireLinkResp response;
                        //if (request==null)
       			//    LOGGER.warning("Object request of Enquire Link is null ");
                        //else
     			LOGGER.warning("Enquire Link request " + request.debugString());
			if (asynchronous) {
                            if (session==null)
       			        LOGGER.warning("Object session of Enquire Link is null ");
                            else
					...
					try {
						...
						case Data.DELIVER_SM:
							DeliverSM deliver = (DeliverSM) request;
                                                        LOGGER.severe(String.format("request DLR: "+ request.debugString()));
							...
							if (dlrInfo == null) {
								LOGGER.severe(String.format("Unable to confirm message based on DLR {%s}", dlr));
                                                                ...
                                                                //LOGGER.warning(String.format("Message DLR is {%s}", dlr));
	...
	protected void storeBasedOnStat(java.sql.Connection conn, OriginCode x, String receiptedMessageId, StatEnum stat,
		...
		if (stat == null) {
			...
			LOGGER.warning(String.format("Stat for {%s} was {%s}", receiptedMessageId, stat));
			switch (stat) {
			case ACCEPTD:
				break;
			case DELIVRD:
				...
				if (Data.SUBMIT_SM_RESP == pdu.getCommandId()) {
					...
					if (x == null) {
						...
						final String smsCode = x.getSmsCode();
                                                //LOGGER.warning("SUBMIT_RESP con smsCode: " + smsCode);
						if (smsCode == null) {
							...
							try (java.sql.Connection conn = m_ds.getConnection()) {
								try {
									...
									if (commandStatus > 0) { // Error
										...
										LOGGER.warning(String.format("******* Submit SM **** %d -> %s {%s} took {%s}", seqN, smsCode,
