...
public class DIDwwClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(DIDwwClient.class.getName());
	...
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "39"));// defaults to Entel's route
	...
	byte dataCoding = 3; // Por defecto ISO-8859-1
	 ...
	 */
	public DIDwwClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final DIDwwClient test = new DIDwwClient()) {
			test.bind();
			if (test.bound) {
			...
			try (java.sql.Connection conn = m_ds.getConnection()) {
				try {
					...
					try (java.sql.PreparedStatement ps = conn.prepareStatement(QUERY)) {
						...
						ps.setInt(4, 11);
                                                //LOGGER.warning(String.format("Consulto BD SMS DIDWW: %s, CODE_RUTA;%d", QUERY,CODE_RUTA) );
								...
								try {
									...
									if (contentMatcher.matches()) {
										...
										request.setSmDefaultMsgId(smDefaultMsgId);
                                                                                ...
                                                                                if ( esm_class==0x40 )
                                                                                {
                                                                                    //LOGGER.warning("ESPECIAL a destination= "+destination+", shortMessage="+shortMessage);
										    int totalSegments = 2; //splittedMsg.length;
											...
											ed.appendString(shortMessage, Data.ENC_CP1252);                                                                                        
                                                                                        ...
                                                                                        //LOGGER.warning(String.format("ed.toString()= "+ed.toString()));
                                                                                }
	...
	private void receive() {
		...
		try {
			...
			if (receiveTimeout == Data.RECEIVE_BLOCKING) {
				LOGGER.fine("The receive is blocking, i.e. the application " + "will stop until a PDU will be received.");
			} else {
