...
public class VTRClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(VTRClient.class.getName());
	...
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "22"));// defaults to VTR's route
	private static final String IDOIDD = System.getProperty("idoidd","310235");
	 ...
	 * @see FileDebug
	 */
	static Debug debug = new FileDebug(dbgDir, "vtr.dbg");
	 ...
	 * @see FileEvent
	 */
	static Event event = new FileEvent(dbgDir, "vtr.evt");
	 ...
	 */
	public VTRClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final VTRClient test = new VTRClient()) {
			test.bind();
			if (test.bound) {
								...
								try {
									...
									if (contentMatcher.matches()) {
										if (IDOIDD.length() == 0) {// Classic mode enabled
											...
											destAddress = new Address((byte) 1, (byte) 1,
