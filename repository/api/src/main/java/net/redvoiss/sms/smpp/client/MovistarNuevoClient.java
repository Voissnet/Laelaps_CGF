...
import static net.redvoiss.sms.smpp.client.MovistarClient.MOBILE_FILTERS;
import org.smpp.util.ByteBuffer;

public class MovistarNuevoClient implements Closeable {
	/**
	...
	//private static final String QUERY = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_udh");
	private static final String QUERY = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_udh_movistar_2"); // leo ta,bien esm_class y udh_jeader
	//private static final String QUERY_FILTER = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_udh_movistar_2"); // Solo puedo enviar a los moviles del filtro
	 ...
	 */
	public MovistarNuevoClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final MovistarNuevoClient test = new MovistarNuevoClient()) {
			test.bind();
			if (test.bound) {
				...
				try {
					...
					final long startTime = System.nanoTime();
                                        String QUERY_STMT = QUERY;
                                            ...
                                            QUERY_STMT += "      AND ROWNUM < 11 AND (a.dst_addrs  in ("+MOBILE_FILTERS+")) and ((TO_CHAR(SYSDATE, 'DY') in ";
	...
	private static void store(java.sql.Connection conn, OriginCode x, String receiptedMessageId, String doneDate)
            ...
            try (java.sql.PreparedStatement ps = conn.prepareStatement(
				"insert into FS_ESME_DATA(SMS_COD,DST_ADDRS,SMS_TXT,FECHA_HORA_PROGRAMADA,CODE_RUTA,CODE_SMSC,ERROR,FECHA_HORA_ENVIADO,SOURC_ADDRS)  " +
                                "select e.SMS_COD,e.DST_ADDRS,e.SMS_TXT,e.FECHA_HORA,e.CODE_RUTA,e.CODE_SMSC,1,TO_DATE(?, 'YYMMDDHH24MI'),e.SOURC_ADDRS from fs_esme e where e.SMS_COD = ? and e.code_ruta = ?  ")) {                    
