...
public class EntelClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(EntelClient.class.getName());
	...
	private static final Pattern MOBILE_NUMBER_PATTERN = Pattern.compile(PREFIX_REGEX + "\\d{9}$");
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "1"));// defaults to Entel's route
	private static final String IDOIDD = System.getProperty("idoidd", "310220");
	 ...
	 * @see FileDebug
	 */
	static Debug debug = new FileDebug(dbgDir, "entel.dbg");
	 ...
	 * @see FileEvent
	 */
	static Event event = new FileEvent(dbgDir, "entel.evt");
	...
	byte dataCoding = 3; // Por defecto ISO-8859-1 //0=GSM7;
	 ...
	 */
	public EntelClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final EntelClient test = new EntelClient()) {
			test.bind();
			if (test.bound) {
						...
						try (java.sql.ResultSet rs = ps.executeQuery()) {
							...
							while (rs.next()) {
								...
								final byte[] udh_header = rs.getBytes("UDH_HEADER");
                                                                //final String msgTXT  = rs.getString("SMS_TXT");
								...
								try {
									...
									if (contentMatcher.matches()) {
										...
										smDefaultMsgId = getParam("Sm default msg id", smDefaultMsgId);
                                                                                ...
                                                                                String nomencoding = Data.ENC_ISO8859_1;
                                                                                dataCoding =  3; // fuerzo Latin1
                                                                                if ("56977667505".equals(rs.getString("DST_ADDRS")))
                                                                                {
                                                                                    dataCoding =  8;
                                                                                    nomencoding = Data.ENC_UTF16;
                                                                                    LOGGER.warning("----> Seteo dataCoding a "+dataCoding+", para destino= "+rs.getString("DST_ADDRS")+",  y texto queda en: '"+rs.getString("SMS_TXT")+"' ");
                                                                                }                                                                                
										...
										request.setReplaceIfPresentFlag(replaceIfPresentFlag);                                                                                
                                                                                request.setShortMessage(shortMessage, nomencoding); //Data.ENC_CP1252); //Data.ENC_ISO8859_1); //Data.ENC_CP1252);                                                                       
										...
										request.setSmDefaultMsgId(smDefaultMsgId);
                                                                                ...
                                                                                LOGGER.warning("----> SMS smsCode="+smsCode+", destino= "+rs.getString("DST_ADDRS")+", esm_class="+esm_class+", dataCoding="+dataCoding+", texto='"+rs.getString("SMS_TXT")+"', dataCoding="+dataCoding);
										...
										for (int i = 0; i < count; i++) {
											...
											LOGGER.fine(String.format("#%d  Submit request %s", i, request.debugString()));
											try {
												if (asynchronous) {
	...
	private void markError(java.sql.Connection conn, String smsCode, String msgIdGW) throws SQLException {
            ...
            //LOGGER.warning("En EntelClient: insert into FS_ESME_DATA (select e.sms_cod, e.dst_addrs, e.sms_txt, e.fecha_hora, e.code_ruta, e.code_smsc, 0 , sysdate, e.sourc_addrs from fs_esme e where e.code_smsc = "+smsCode+"   )");            
            //try (java.sql.PreparedStatement ps = conn.prepareStatement(
	...
	private void submitMulti() {
		debug.enter(this, "SMPPTest.submitMulti()");
                LOGGER.warning("En SMPPTest.submitMulti()");
								...
								if (receiptedMessageId == null) {
									...
									try (java.sql.Connection conn = m_ds.getConnection()) {
										try {
											...
											if (x == null) {
                                                                                            LOGGER.warning("select e.sms_cod, e.msgid_gw from fs_esme e where e.code_smsc = '"+receiptedMessageId+"' and e.code_ruta = "+CODE_RUTA);
	...
	private static void store(java.sql.Connection conn, OriginCode x, String receiptedMessageId, String doneDate)
            ...
            /*try (java.sql.PreparedStatement ps = conn.prepareStatement(
				"insert into FS_ESME_DATA(SMS_COD,DST_ADDRS,SMS_TXT,FECHA_HORA_PROGRAMADA,CODE_RUTA,CODE_SMSC,ERROR,FECHA_HORA_ENVIADO,SOURC_ADDRS,ESM_CLASS,UDH_HEADER)  " +
                                "select e.SMS_COD,e.DST_ADDRS,e.SMS_TXT,e.FECHA_HORA,e.CODE_RUTA,e.CODE_SMSC,1,sysdate,e.SOURC_ADDRS,e.ESM_CLASS,e.UDH_HEADER from fs_esme e where e.SMS_COD = ?  and e.code_ruta = ? )")) {
*/
            
           //LOGGER.warning (" --> insert into FS_ESME_DATA (select e.sms_cod, e.dst_addrs, e.sms_txt, e.fecha_hora, e.code_ruta, e.code_smsc, 0, TO_DATE('"+doneDate+"', 'YYMMDDHH24MI'), e.sourc_addrs from fs_esme e where e.code_smsc = '"+receiptedMessageId+"' and e.code_ruta = "+CODE_RUTA+" )");
