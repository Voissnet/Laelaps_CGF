...
import static net.redvoiss.sms.smpp.client.MovistarNuevoClient.MOBILE_FILTERS;
import org.smpp.util.ByteBuffer;

public class MovistarClient implements Closeable {
	/**
	 * Revoiss instance variables
	 */
	private DataSource m_ds;
	private static final Logger LOGGER = java.util.logging.Logger.getLogger(MovistarClient.class.getName());
	...
	protected static final Pattern MOBILE_NUMBER_PATTERN = Pattern.compile(PREFIX_REGEX + "\\d{9}$");
	private static final int CODE_RUTA = Integer.parseInt(System.getProperty("route", "9"));
	private static final String IDOIDD = System.getProperty("idoidd", "");
	...
	private static final String QUERY = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_udh"); // leo ta,bien esm_class y udh_jeader
	private static final String QUERY_FILTER = DAO.loadProperties().getProperty("net.redvoiss.sms.dao.check.sms_udh_movistar_1"); // este impide enviar los moviles en el filtro
        
        // CGF, 20210826, El siguiente campso es usado para levantar el conector y que solo rescate los SMS al destino 
        // dado segun el movil en MOBILE_FILTER y que es requerido para probar la nueva conexion a MOVISTAR.
	public static  String MOBILE_FILTERS = DAO.loadProperties().getProperty("mobile_filter", "");
	 ...
	 * @see FileDebug
	 */
	static Debug debug = new FileDebug(dbgDir, "movistar.dbg");
	 ...
	 * @see FileEvent
	 */
	static Event event = new FileEvent(dbgDir, "movistar.evt");
	...
	/**
	 * Contains the parameters and default values for this test application such as
	 * system id, password, default npi and ton of sender etc.
	 */
	...
	/**
	 * If the application has to keep reading commands from the keyboard and to do
	 * what's requested.
	 */
	 ...
	 * transciever (tr). Transciever can both send messages and receive messages.
	 * Note, that if you bind as receiver you can still receive responses to you
	 * requests (submissions).
	 */
	String bindOption = "t";

	/**
	 * Indicates that the Session has to be asynchronous. Asynchronous Session means
	 * that when submitting a Request to the SMSC the Session does not wait for a
	 * response. Instead the Session is provided with an instance of implementation
	 * of ServerPDUListener from the smpp library which receives all PDUs received
	 * from the SMSC. It's application responsibility to match the received Response
	 * with sended Requests.
	 */
	boolean asynchronous = false;

	/**
	 * This is an instance of listener which obtains all PDUs received from the
	 * SMSC. Application doesn't have explicitly call Session's receive() function,
	 * all PDUs are passed to this application callback object. See documentation in
	 * Session, Receiver and ServerPDUEventListener classes form the SMPP library.
	 */
	...
	/**
	 * Initialises the application, lods default values for connection to SMSC and
	 * for various PDU fields.
	 */
	public MovistarClient() throws IOException, SQLException {
	...
	public static void main(String args[]) throws Exception {
		...
		try (final MovistarClient test = new MovistarClient()) {
			...
			if (test.bound) {
				...
				}, 0, test.getIntProperty("enquire-link-recurrence-period", 5), TimeUnit.SECONDS);
				test.submit();
			...
			LOGGER.log(SEVERE, "Exception initialising SMPPTest ", e);
		} finally {
	...
	public void close() {
		...
		try {
			...
			LOGGER.log(WARNING, "Close interrupted", ie);
		}
	}
	...
	private void bind() {
		...
		try {
			...
			// send the request
                        LOGGER.severe("Bind request to ipAddress: "+ipAddress+", port: "+port);
			LOGGER.severe("Bind request " + request.debugString());
			if (asynchronous) {
			...
			LOGGER.severe("Bind response " + response.debugString());
	...
	/**
	 * Creates a new instance of <code>SubmitSM</code> class, lets you set subset of
	 * fields of it. This PDU is used to send SMS message to a device.
	 *
			...
			try (java.sql.Connection conn = m_ds.getConnection()) {
				try {
					...
					final long startTime = System.nanoTime();
                                        ...
                                        if (!"".equals(MOBILE_FILTERS)) 
                                        {
                                            LOGGER.warning("Filtro MOBILE_FILTERS:"+MOBILE_FILTERS);
                                            QUERY_STMT = QUERY_FILTER;
                                            QUERY_STMT = "SELECT a.sms_cod, a.dst_addrs, a.sms_txt, a.fecha_hora, a.status, a.enviado, a.retry, ";
                                            QUERY_STMT += "      a.code_ruta, a.code_smsc, a.error, a.sourc_addrs, a.msgid_gw, a.iterator, a.prior_cod, ";
                                            QUERY_STMT += "      a.dest_cod, a.comercial_type, a.esm_class, a.udh_header FROM FS_ESME a ";
                                            QUERY_STMT += "      WHERE a.fecha_hora < sysdate AND a.status = "+DAO.MessageState.NEW.getState()+" AND a.code_ruta in ("+CODE_RUTA+","+CODE_RUTA+") AND a.error = 0 ";
                                            QUERY_STMT += "      AND ROWNUM < 11 AND (a.dst_addrs not in ("+MOBILE_FILTERS+")) and ((TO_CHAR(SYSDATE, 'DY') in ";
                                            QUERY_STMT += "      ('MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT') and (extract(hour from cast(sysdate as timestamp)) ";
                                            QUERY_STMT += "      BETWEEN 8 and 19 or (a.comercial_type = 0 and extract(hour from cast(sysdate as timestamp)) ";
                                            QUERY_STMT += "      not BETWEEN 8 and 19))) OR (TO_CHAR(SYSDATE, 'DY') in ('SUN') and a.comercial_type = 0))     ";
                                            QUERY_STMT += "      ORDER BY a.prior_cod DESC, a.fecha_hora DESC, a.sms_cod ASC  FOR UPDATE";
                                        } else {
                                            QUERY_STMT = "SELECT a.sms_cod, a.dst_addrs, a.sms_txt, a.fecha_hora, a.status, a.enviado, a.retry, ";
                                            QUERY_STMT += "      a.code_ruta, a.code_smsc, a.error, a.sourc_addrs, a.msgid_gw, a.iterator, a.prior_cod, ";
                                            QUERY_STMT += "      a.dest_cod, a.comercial_type, a.esm_class, a.udh_header FROM FS_ESME a ";
                                            QUERY_STMT += "      WHERE a.fecha_hora < sysdate AND a.status = "+DAO.MessageState.NEW.getState()+" AND a.code_ruta in ("+CODE_RUTA+","+CODE_RUTA+") AND a.error = 0 ";
                                            QUERY_STMT += "      AND ROWNUM < 11 AND  ((TO_CHAR(SYSDATE, 'DY') in ";
                                            QUERY_STMT += "      ('MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT') and (extract(hour from cast(sysdate as timestamp)) ";
                                            QUERY_STMT += "      BETWEEN 8 and 19 or (a.comercial_type = 0 and extract(hour from cast(sysdate as timestamp)) ";
                                            QUERY_STMT += "      not BETWEEN 8 and 19))) OR (TO_CHAR(SYSDATE, 'DY') in ('SUN') and a.comercial_type = 0))     ";
                                            QUERY_STMT += "      ORDER BY a.prior_cod DESC, a.fecha_hora DESC, a.sms_cod ASC  FOR UPDATE";
                                        }
                                        //LOGGER.warning("*** >>> SQL="+QUERY_STMT);
                                        try (java.sql.PreparedStatement ps = conn.prepareStatement(QUERY_STMT)) {
						/*ps.setString(1, DAO.MessageState.NEW.getState());
						ps.setInt(2, CODE_RUTA);
						ps.setInt(3, CODE_RUTA);
						ps.setInt(4, 11);   
                                                if (!"".equals(MOBILE_FILTERS)) 
                                                {
                                                    ps.setString(5,MOBILE_FILTERS);
                                                }*/
						...
						try (java.sql.ResultSet rs = ps.executeQuery()) {
							...
							while (rs.next()) {
								...
								try {
									...
									java.util.regex.Matcher contentMatcher = MOBILE_NUMBER_PATTERN.matcher(destination);
									if (contentMatcher.matches()) {
										if (IDOIDD.length() == 0) {// Classic mode enabled
											destAddress = new Address((byte) 1, (byte) 1, destination);
										} else {
											destAddress = new Address((byte) 0, (byte) 13,
													destination.replaceFirst(PREFIX_REGEX, IDOIDD));
										}
										...
										request.setEsmClass(esm_class); //esmClass);
										...
										for (int i = 0; i < count; i++) {
											...
											try {
												if (asynchronous) {
													...
													try (java.sql.PreparedStatement _ps = conn.prepareStatement(
															"UPDATE FS_ESME SET ITERATOR=? where SMS_COD=?")) {
														_ps.setInt(1, seq);
														_ps.setString(2, smsCode);
														int rowCount = _ps.executeUpdate();
														assert rowCount > 0;
													}
												} else {
													SubmitSMResp response = session.submit(request);
													LOGGER.fine("Submit response " + response.debugString());
													messageId = response.getMessageId();
												}
												try (java.sql.PreparedStatement _ps = conn.prepareStatement(
														"UPDATE FS_ESME SET FECHA_HORA=sysdate, STATUS=? where SMS_COD=?")) {
													_ps.setString(1, DAO.MessageState.PENDING.getState());
													_ps.setString(2, smsCode);
													int rowCount = _ps.executeUpdate();
													assert rowCount > 0;
												}
	...
	/**
	 * Creates a new instance of <code>ReplaceSM</code> class, lets you set subset
	 * of fields of it. This PDU is used to replace certain attributes of already
	 * submitted message providing that you 'remember' message id of the submitted
	 * message. The message id is assigned by SMSC and is returned to you with the
	 * response to the submision PDU (SubmitSM, DataSM etc.).
	 *
	...
	/**
	 * Creates a new instance of <code>CancelSM</code> class, lets you set subset of
	 * fields of it. This PDU is used to cancel an already submitted message. You
	 * can only cancel a message which haven't been delivered to the device yet.
	 *
	...
	/**
	 * Creates a new instance of <code>DataSM</code> class, lets you set subset of
	 * fields of it. This PDU is an alternative to the <code>SubmitSM</code> and
	...
	/**
	 * Creates a new instance of <code>QuerySM</code> class, lets you set subset of
	 * fields of it. This PDU is used to fetch information about status of already
	 * submitted message providing that you 'remember' message id of the submitted
	 * message. The message id is assigned by SMSC and is returned to you with the
	 * response to the submision PDU (SubmitSM, DataSM etc.).
	 *
	...
	private void receive() {
		...
		try {
			...
			if (pdu != null) {
				LOGGER.warning("Received PDU " + pdu.debugString());
				if (pdu.isRequest()) {
					...
					try {
						...
						case Data.DELIVER_SM:
							...
							final String dlr = deliver.getShortMessage();
							final DLRInfo dlrInfo = SMPPClient.getDLRInfo(dlr);
							if (dlrInfo == null) {
								...
								if (deliver.hasReceiptedMessageId()) {
									...
									if ((receiptedMessageId = dlrInfo.getId()) == null) {
										LOGGER.warning(String.format("Message does not comply DLR format: {%s}", dlr));
                                                                ...
                                                                //LOGGER.warning("Received  deliver.getReceiptedMessageId()="+receiptedMessageId+", dlrInfo.getId()="+dlrInfo.getId());
								if (receiptedMessageId == null) {
									LOGGER.warning("Unable to get receipted message id (FS_ESME.CODE_SMSC)");
								} else {
									//LOGGER.warning(String.format("CODE_SMSC should be {%s}", receiptedMessageId));
									OriginCode x = m_smsCodeFromSMSCID.remove(receiptedMessageId);
									try (java.sql.Connection conn = m_ds.getConnection()) {
										try {
											conn.setAutoCommit(false);
											final StatEnum stat = dlrInfo.getStat();
											if (x == null) {
												try (java.sql.PreparedStatement ps = conn.prepareStatement(
	...
	private static void store(java.sql.Connection conn, OriginCode x, String receiptedMessageId, String doneDate)
			throws SQLException {    
            try (java.sql.PreparedStatement ps = conn.prepareStatement(
				"insert into FS_ESME_DATA(SMS_COD,DST_ADDRS,SMS_TXT,FECHA_HORA_PROGRAMADA,CODE_RUTA,CODE_SMSC,ERROR,FECHA_HORA_ENVIADO,SOURC_ADDRS)  " +
                                "select e.SMS_COD,e.DST_ADDRS,e.SMS_TXT,e.FECHA_HORA,e.CODE_RUTA,e.CODE_SMSC,0,TO_DATE(?, 'YYMMDDHH24MI'),e.SOURC_ADDRS from fs_esme e where e.CODE_SMSC = ? and e.code_ruta = ? ")) {
					...
					if (x == null) {
						...
						if (smsCode == null) {
							...
							try (java.sql.Connection conn = m_ds.getConnection()) {
								...
								LOGGER.log(SEVERE,
										String.format("Error trying to match sequence and sms code {%s}", smsCode), e);
							}
						}
					}
				}
	...
	/**
	 * Prompts the user to enter an address value with max length set to the default
	 * length Data.SM_ADDR_LEN.
	 */
