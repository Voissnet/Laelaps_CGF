...
@DataSourceDefinition(
    ...
    name = "java:app/jdbc/myOracleDesaDatasource",
    user = "FG",
    password = "FG",
    databaseName = "DESARROLLO",
    portNumber=1521,
    properties={"url=jdbc:oracle:thin:@localhost:1521:DESARROLLO"}
) 
*/
@Singleton
@Startup
public class SMSCBean {
    ...
    @PostConstruct
    public void init() {
        LOGGER.severe("QA Server will ini SMSCBean ");
        if (m_ds == null) {
            ...
            if (Boolean.parseBoolean(System.getenv("SMSC_DISABLED"))) {
                ...
                try {
                    ...
                    m_ds = (DataSource) ctx.lookup( "jdbc/DB_DESA" );                    
